...
class NexusApp(MDApp):
    ...
    def create_post(self):
        try:
            ...
            if ok:
                print("Post saved")
                self.root.ids.post_input.text = ""
                self.add_notification("Post Published", "Your post is live", "success")
    ...
    def add_post_from_tab(self):
        try:
            ...
            safe, issues = ContentGuard.check(text)
            if not safe:
                self.root.ids.add_status.text = "Content not allowed"
                print(f"Flagged: {issues}")
                return
            post_data = {
                "userId": self.current_user["localId"],
    ...
    def audio_call(self):
        try:
            ...
            self.add_notification("Audio Call", f"Calling {name}", "call")
        except Exception as e:
    ...
    def video_call(self):
        try:
            ...
            self.call_seconds = 0
            Clock.schedule_once(self.start_call_timer, 2)
            self.add_notification("Video Call", f"Calling {name}", "call")
        except Exception as e:
    ...
    def add_notification(self, title, message, ntype="info"):
        if not self.notifications_on:
            return None
        notif = self.notif.add(title, message, ntype)
        try:
            self.load_notifications()
        except Exception:
            pass
    ...
    def save_new_name(self, dlg):
        try:
            ...
            self.add_notification("Name Updated", f"Now: {new_name}", "success")
            dlg.dismiss()
